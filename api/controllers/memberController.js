...
exports.list_all_members = (req, res) => {
    Member.find({ project_id: req.params.project_id }, (error, members) => {
        if (error) {
...
exports.create_a_member = (req, res) => {
    Project.findById(req.params.project_id, (error, project) => {
        if (error) {
            ...
            let new_member = new Member({ project_id: req.params.project_id, ...req.body });
            new_member.save((error, member) => {
                if (error) {
                    routeProvider.generateError(500, 'Erreur serveur', res);
                }
                else {
...
exports.get_a_member = (req, res) => {
    ...
    Member.findById(req.params.member_id, (error, member) => {
        if (error) {
            routeProvider.generateError(500, 'Erreur serveur', res);
        }
        else {
...
exports.update_a_member = (req, res) => {
    Member.findByIdAndUpdate(req.params.member_id, req.body, { new: true, useFindAndModify: false }, (error, member) => {
        if (error) {
            routeProvider.generateError(500, 'Erreur serveur', res);
        }
        else {
...
exports.delete_a_member = (req, res) => {
    Member.findByIdAndRemove(req.params.member_id, (error) => {
        if (error) {
            routeProvider.generateError(500, 'Erreur serveur', res);
        }
        else {
