...
// Generate methods fro CRUD project
...
{
    // Get all projects when member limit is < 5
    Project.find(
        { 
            school_id: req.params.school_id 
        },
        (error, projects) => 
        {
            ...
            else 
            {
                ...
                if (projects.length > 0) 
                {
                    ...
                    {
                        //  for each project, get list member
                        Member.find(
                            { 
                            ...
                            (error, members) => 
                            {
                                ...
                                {
                                    // limit member < 5, is OK
                                    // push project in tab result 
                                    projectsFilter.push(project);

                                    if (i === projects.length) 
                                    {
                                        // check all member is test
